...
function MdTabInkDirective($mdEffects, $window, $$rAF, $timeout) {
  ...
  function postLink(scope, element, attr, ctrls) {
    ...
    scope.$on('$mdTabsEditChanged', debouncedUpdateBar);
    scope.$on('$mdTabsEditPaginationChanged', debouncedUpdateBar);
...
.directive('mdTabsEditPagination', [
  '$mdEffects',
  '$window',
  '$$rAF',
  '$$q',
...
function TabPaginationDirective($mdEffects, $window, $$rAF, $$q, $timeout) {
  ...
  var TAB_MIN_WIDTH = 8 * 10; 

  // Must match tab min-width rule in _tabs.scss
  // var PAGINATOR_RIGHT = 42;
  // var PAGINATOR_LEFT = 62;

  var PAGINATORS_WIDTH = 0;
  ...
  function postLink(scope, element, attr, tabsCtrl) {
    ...
    scope.$on('$mdTabsEditChanged', debouncedUpdatePagination);
    ...
    function updatePagination() {
      ...
      var tabsWidth = element[0].offsetWidth - PAGINATORS_WIDTH;

      console.log(element[0], element[0].offsetWidth);
    ...
    function setPage(page) {
        ...
        $timeout(function() {
            scope.$broadcast('$mdTabsEditPaginationChanged');
        });
...
.controller('$mdTabsEdit', [
  '$scope', 
  '$element',
  '$mdUtil',
  MdTabsEditController
]);

function MdTabsEditController(scope, element, $mdUtil) {
  ...
  function add(tab, index) {
    ...
    scope.$broadcast('$mdTabsEditChanged');
  }
